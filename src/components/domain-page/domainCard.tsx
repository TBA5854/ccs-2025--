...
const DomainCard: FC<domainCardProps & { loading: boolean }> = ({
  ...
  return (
    <div
      className={`border-4  rounded-[6px] shadow-md p-4 flex flex-col h-full justify-between ${getCardBorders(domainName)}`}
    >
      ...
      >
        {loading ? (
          ...
          <button
            disabled={disabled}
            type="button"
            className={`mt-4 py-1.5 px-0 w-full min-h-[40px] h-fit border border-[rgba(240,246,252,0.10)] rounded-[6px] transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-gray-400 text-[13px] font-sans font-medium leading-[20px] text-center
